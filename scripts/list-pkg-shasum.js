 ...
 * registry mismatch.
 *
 * --publish runs the real npm publish interactively (stdio inherited), so accounts
 * with 2FA get the browser web-auth prompt per package. The published tarball is
 * verified against the registry shasum after each publish.
 *
...
// The real publish runs npm interactively so 2FA accounts get the browser
// web-auth flow. That requires a real terminal; fail fast (before the dry-run
// pass) instead of erroring on the first publish in CI or a piped shell.
if (PUBLISH_FILE && (!process.stdin.isTTY || !process.stdout.isTTY)) {
    console.error(
        '--publish requires an interactive terminal (TTY) so npm can run the browser web-auth 2FA flow. ' +
            'Run it from an interactive shell, not CI or a piped process.'
    );
    process.exit(1);
}
...
// A just-published version can briefly 404 while the registry syncs; retry a few
// times before concluding it is missing.
async function getRegistryShasumWithRetry(name, version, registry = REGISTRY, attempts = 3, delayMs = 5000) {
    let lastError = null;

    for (let attempt = 0; attempt < attempts; attempt++) {
        try {
            const shasum = await getRegistryShasum(name, version, registry);
            if (shasum) return shasum;
            // The request succeeded but the version/shasum is not visible yet.
            lastError = null;
        } catch (error) {
            lastError = error;
        }
...
function runNpmPublish(tempDir, { dryRun = false, registry = null, interactive = false } = {}) {
    // Interactive mode hands npm a real TTY (stdio inherit), so accounts with 2FA
    // get the browser web-auth prompt instead of a hard EOTP failure. npm's JSON
    // output cannot be captured in this mode, so it returns null; callers must
    // verify the published tarball via the registry shasum instead.
    const npmArgs = interactive
        ? ['publish', '--tag', TAG, '--access', 'public']
        : ['publish', '--json', '--tag', TAG, '--access', 'public'];
...
function publishPackage(pkg, registry, { interactive = false } = {}) {
    return withPreparedPackage(pkg, (tempDir, frozenDependencies) => {
        try {
            const info = runNpmPublish(tempDir, { registry, interactive });
            return {
                name: pkg.name,
                version: pkg.version,
                shasum: info ? info.shasum : null,
...
async function publishManifest(file, registry = REGISTRY) {
    ...
    for (const { pkg, name, version, shasum } of publishQueue) {
        ...
        try {
            ...
            console.log('failed');
            console.error(`Unable to query registry before publishing ${id}: ${e.message || e}`);
            publishRows.push([name, version, shasum, '(not attempted)', '(query failed)', 'MISMATCH']);
            continue;
        }

        if (existingVersion === version) {
            publishInfo = { alreadyPublished: true, shasum: null };
        } else {
            try {
                // The interactive publish inherits stderr, so EPUBLISHCONFLICT cannot
                // be detected from the error output. The preflight check above avoids
                // the common conflict case; the registry recovery below handles races
                // and npm errors that occur after the tarball was accepted.
                publishInfo = publishPackage(pkg, registry, { interactive: true });
            } catch (e) {
                let recoveredShasum = null;
                let recoveryError = null;
                try {
                    recoveredShasum = await getRegistryShasumWithRetry(name, version, registry);
                } catch (error) {
                    recoveryError = error;
                }

                if (recoveredShasum === shasum) {
                    console.log('OK (recovered after npm error)');
                    console.warn(
                        `npm exited with an error for ${id}, but the registry already has the expected shasum; ` +
                            `treating as published. Original error: ${e.message || e}`
                    );
                    publishRows.push([name, version, shasum, '(recovered after error)', recoveredShasum, 'OK']);
                    continue;
                }

                console.log('failed');
                console.error(`npm publish failed for ${id}: ${e.message || e}`);
                if (recoveryError) {
                    console.error(
                        `Unable to verify registry after the npm error: ${recoveryError.message || recoveryError}`
                    );
                } else if (recoveredShasum) {
                    console.error(
                        `Registry has ${id} with shasum ${recoveredShasum}, which does not match the manifest ` +
                            `(${shasum}); this is a real conflict, not a transient failure.`
                    );
                }
                publishRows.push([
                    name,
                    ...
                    shasum,
                    '(error)',
                    recoveryError ? '(query failed)' : recoveredShasum || '(missing)',
                    'MISMATCH',
                ]);
                continue;
            }
        ...
        try {
            registryShasum = await getRegistryShasumWithRetry(name, version, registry);
        } catch (e) {
            console.log('failed');
            console.error(
                `${publishInfo.alreadyPublished ? 'Package already exists' : 'npm publish completed'}, ` +
                    `but registry verification failed for ${id}: ${e.message || e}`
            );
            publishRows.push([
                name,
                version,
                shasum,
                publishInfo.alreadyPublished ? '(already published)' : '(interactive)',
                '(query failed)',
                'MISMATCH',
            ]);
            continue;
        ...
        if (!registryShasum) {
            console.error(`Registry did not expose a shasum for ${id} after all retry attempts.`);
        } else if (registryShasum !== shasum) {
            console.error(
                `Registry has ${id} with shasum ${registryShasum}, which does not match the manifest (${shasum}).`
            );
        }

        // Interactive publishes report shasum as null (no JSON output); the registry
        // shasum below is the authoritative check for what was actually uploaded.
        const publishMatches =
            publishInfo.alreadyPublished || publishInfo.shasum === null || publishInfo.shasum === shasum;
        ...
        publishRows.push([
            ...
            version,
            shasum,
            publishInfo.alreadyPublished ? '(already published)' : publishInfo.shasum || '(interactive)',
            registryShasum || '(missing)',
            status,
        ]);
    }
...
async function main() {
    ...
    if (OUT_FILE) {
        ...
        const manifest = {
            ...
            packageManager: 'npm',
            publishCommand: `npm publish --tag ${TAG} --access public`,
